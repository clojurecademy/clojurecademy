(ns clojurecademy.middleware.anti-forgery
  ...
  *anti-forgery-token* var, which is bound to a random key unique to the
  ...
  (fn [request]
    (binding [*anti-forgery-token* (or (session-token request) (new-token))]
      (if (and (not (get-request? request)) (not (valid-request? request read-token)))
        (if (in? (:uri request) (:ignored-paths options))
          (handler request)
          (handle-error options request))
        (some-> (handler request) (assoc-session-token request *anti-forgery-token*))))))
